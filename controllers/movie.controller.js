...
const popularFile = path.resolve(__dirname, "../popular_movies.json");
...
const popular = JSON.parse(fs.readFileSync(popularFile, "utf-8"))
    ...
    let movie = await Movie.findOne({
      ...
      include: [
        ...
        },
        {
          ...
          attributes: ['id', 'title', 'slug', 'genres', 'posterUrl', 'backdropUrl', 'synopsis', 'releaseDate', 'runtime', 'type', 'mpaRating', 'voteAverage', 'f_id', 'tmdbId', 'voteCount'],
...
const getPopularMovieDetails = async (ids) => {
  ...
  let movies = await Movie.findAll({
    ...
    attributes: ['id', 'title', 'slug', 'genres', 'posterUrl', 'backdropUrl', 'synopsis', 'synopsis', 'releaseDate', 'runtime', 'type', 'mpaRating', 'voteAverage', 'f_id', 'tmdbId', 'voteCount'],
  });
...
exports.getPopularMovies = async (req, res, next) => {
  ...
  const { page, totalPages, items } = await paginateArray(ids, currentPage, limit);
  const detailed = await getPopularMovieDetails(items);

  res.json({ category: 'popular', page, totalPages, movies: detailed });
...
exports.search = async (req, res) => {
  ...
  try {
    ...
    const searched = await fSearch(titleQuery);

    return res.json({
      success: true,
      movies: (searched?.results || []).map(m => {
        m.f_id = m.id;
        m.releaseDate = m.releaseDate.replace("$D", "");
        delete m.id;
        return m;
      })
    });
