...
const { Movie, Series } = require('./models'); // Ensure Movie is exported in models/index.js
...
async function scrapeAll(req, res) {
    ...
    // Fetch pages in batches of 10 concurrently
    ...
    for (let i = 2; i <= totalPages; i += concurrency) {
        ...
        await new Promise((r) => setTimeout(r, 2000)); // short pause to avoid rate limit
    }
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        await Promise.all(
            batch.map(async (movie) => {
                try {
                    if (!movie.id) return; // skip invalid data
                    ...
                    let movieAdded = await Movie.findOne({
                        where: { f_id: movie.id },
                    });

                    if (!movieAdded) {

                        movieAdded = await Movie.create({
                            f_id: movie.id,
                            title: movie.title || 'Untitled',
                            slug,
                            genres: [],
                            posterUrl: movie.posterUrl || null,
                            year: movie.year || new Date(movie.releaseDate.replace("$D", "")).getFullYear(),
                            releaseDate: movie.releaseDate.replace("$D", "") || null,
    ...
    // Save all results
    fs.writeFileSync("popular_movies.json", JSON.stringify(successfulMovies, null, 2));
    console.log(`\n🎉 Done! Saved ${successfulMovies.length} movies to popular_movies.json`);
    ...
    if (res) res.json({ success: true, message: `Scraped ${successfulMovies.length} movies.` });
